...
class Stereo : public nodelet::Nodelet {
  public:
    ...
    void onInit() {
      ...
      left_info_mgr = new CameraInfoManager(ros::NodeHandle(node, "left"), "left_camera");
      right_info_mgr = new CameraInfoManager(ros::NodeHandle(node, "right"), "right_camera");
      ...
      ok = true;
      image_thread = boost::thread(boost::bind(&Stereo::feedImages, this));
    }
    ...
    ~Stereo() {
      ...
      image_thread.join();
      if (it) delete it;
      if (cam_left) delete cam_left;
      if (cam_right) delete cam_right;
    }
...
PLUGINLIB_DECLARE_CLASS(uvc_camera, Stereo, uvc_camera::Stereo, nodelet::Nodelet);
